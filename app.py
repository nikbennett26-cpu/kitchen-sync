...
<style>
    ...
    .sub-text {{ color: {sub_text} !important; font-size: 0.9rem; }}

    /* 3. SIDEBAR (Mobile Fix) */
    ...
    /* 4. INPUT BOX (THE FIX) */
    /* Target the container and the input itself */
    ...
    .stTextArea textarea {{
        ...
        caret-color: {text_color}; /* Fix cursor color */
        -webkit-text-fill-color: {input_text} !important;
        border: none !important; /* Let container handle border */
    }}
    ...
    [data-testid="stVerticalBlockBorderWrapper"]:hover {{
        ...
        box-shadow: 0 2px 0 {card_border} !important;
    }}
    ...
    for i, item in enumerate(matches):
        ...
        with (col1 if i % 2 == 0 else col2):
            with st.container(border=True):
                ...
                st.markdown(f'<div class="sub-text">⏱️ {recipe.get("time", "--")}</div>', unsafe_allow_html=True)
